...
CATEGORY = os.environ.get("CATEGORY", "pcb1")
MODEL_DIR = os.environ.get("MODEL_DIR", "/models")
TRAIN_DIR = os.environ.get("TRAIN_DIR", f"/train/{os.environ.get('CATEGORY', 'pcb1')}")
...
DETECTOR = EfficientAdDetector(MODEL_DIR, TRAIN_DIR, CATEGORY, DEVICE)

# One model, one device, every camera. Scoring is serialised so concurrent
# streams cannot interleave on the GPU -- which is also what makes the
# per-camera CUDA-event attribution in gpu.py add up rather than overlap.
...
def _startup():
    how = DETECTOR.ensure()
    print(f"detector {how}: category={CATEGORY} threshold={DETECTOR.threshold:.4f} "
